...
var DEPS_KEYWORD_STRING_PATTERN = '[ \\t]*([\\w-]+)[ \\t]*';
var DEPS_KEYWORD_STRING_REGEX = RegExp(DEPS_KEYWORD_STRING_PATTERN);
var DEPS_EASY_GAV_STRING_REGEX = RegExp(
  '(["\']?)([\\w.-]+):([\\w.-]+)[:]?([\\w\\[\\]\\(\\),+.-]+)?\\1',
);
var DEPS_HARD_GAV_STRING_REGEX = RegExp(
  DEPS_KEYWORD_STRING_PATTERN + '(?:\\((.*)\\)|(.*))',
);
var DEPS_ITEM_BLOCK_REGEX = RegExp(
  DEPS_KEYWORD_STRING_PATTERN + '\\(((["\']?)(.*)\\3)\\)[ \\t]*\\{',
);
var DEPS_EXCLUDE_LINE_REGEX = RegExp('exclude[ \\t]+([^\\n]+)', 'g');
var PLUGINS_LINE_PATTERN = RegExp(
  '(id|version)[ \\t]*\\(?(["\']?)([A-Za-z0-9._-]+)\\2\\)?',
  'g',
);
...
function deepParse(chunk, state, keepFunctionCalls, skipEmptyValues) {
  ...
  for (; state.index < chunkLength; state.index++) {
    ...
    if (
      parsingKey &&
      !keepFunctionCalls &&
      character === CHAR_LEFT_PARENTHESIS
    ) {
    ...
    if (character === CHAR_BLOCK_START) {
      ...
      if (SPECIAL_KEYS.hasOwnProperty(currentKey)) {
        ...
        out[currentKey] = deepAssign(
          {},
          out[currentKey],
          deepParse(chunk, state, keepFunctionCalls, skipEmptyValues),
        );
      } else {
        out[currentKey] = deepParse(
          chunk,
          state,
          keepFunctionCalls,
          skipEmptyValues,
      ...
      isBeginningOfLine =
        isBeginningOfLine &&
        (character === CHAR_SLASH || isStartOfComment(tempString));
    }
  }
...
function findDependencyItemBlock(data) {
  ...
  if (matches && matches[2]) {
    ...
    while ((match = DEPS_EXCLUDE_LINE_REGEX.exec(data))) {
      excludes.push(
        parseMapNotation(
          match[0].substring(findFirstSpaceOrTabPosition(match[0])),
        ),
      );
    }
...
function parseGavString(gavString) {
  ...
  if (easyGavStringMatches) {
    ...
    if (hardGavMatches && (hardGavMatches[3] || hardGavMatches[2])) {
      out = parseMapNotationWithFallback(
        out,
        hardGavMatches[3] || hardGavMatches[2],
      );
    } else {
      out = parseMapNotationWithFallback(
        out,
        gavString,
        gavString.slice(findFirstSpaceOrTabPosition(gavString)),
      );
    }
  }
  return out;
...
function parseSpecialClosure(chunk, state, mapFunction) {
  ...
  for (; state.index < chunk.length; state.index++) {
    if (chunk[state.index] === CHAR_BLOCK_START) {
      ...
      if (CHAR_LEFT_PARENTHESIS === chunk[state.index]) {
        currentValue = '(';
      } else {
    ...
    if (
      currentKey &&
      (isWhitespace(chunk[state.index]) ||
        CHAR_LEFT_PARENTHESIS === chunk[state.index])
    ) {
      var character = '';
      for (
        state.index = state.index + 1;
        ...
        // if (chunk[state.index] !== CHAR_RIGHT_PARENTHESIS) {}
...
function isStartOfComment(snippet) {
  return (
    snippet === BLOCK_COMMENT_START || snippet === SINGLE_LINE_COMMENT_START
  );
}

function isCommentComplete(text, next) {
  return (
    (isLineBreakCharacter(next) && isSingleLineComment(text)) ||
    (isWhitespace(next) && isEndOfMultiLineComment(text))
  );
}
